...
const $bgMenuIcon = document.getElementById("bg-menu-icon");
...
const $cartBtn = document.querySelectorAll(".js-cart-btn");
...
const $closeCartBtn = document.querySelectorAll(".js-close-cart-btn");
...
const $renderCartMenu = document.getElementById("render-cart-menu");
// Selecciona los lugares en donde se mostrará el número de productos adquiridos
const $purchaseCount = document.querySelectorAll(".js-purchase-count");
...
function getCategoryProducts(products, category) {
    ...
    products.forEach((productItem) => {
        if (productItem.category === category)
            featuredProducts.push(productItem);
    });
...
function createProductCard(product, category) {
    ...
    const $renderProducts = document.getElementById(`render-${category}`);
    ...
    for (let i = 0; i < product.length; i++) {
        html += `
    <article class="card ${product[i].category === "bouquets" ? "card--3-col" : ""}">
    <div class="card__header">
    <a href="#" class="card__image-container">
    <img src="${product[i].image}" alt="${product[i].name}" class="card__image" loading="lazy" />
    </a>
    ...
    <button data-product-index="${
        ...
        product[i].id}" class="js-add-to-cart add-to-cart-btn">Añadir al carrito</button>
    </div>

    <div class="card__body">
    <h3 class="heading-3 u-margin-top-xs"><a href="#" class="card__heading">${product[i].name}</a></h3>
    <h4 class="heading-4 u-margin-top-xs">
    ${
        // Si el producto tiene descuento, el precio original se muestra tachado
        product[i].discount
            ? `<span class="card__original-price">${product[i].price.toFixed(2)}€</span>`
            : ""}

    ${
        // Si el producto tiene descuento se realiza la operación y se muestra el precio final de color magenta
        product[i].discount
            ? `<span class="card__actual-price">${(product[i].price -
                (product[i].price * product[i].discount) / 100).toFixed(2)}€</span>`
            : ""}
...
<div class="cart-menu__product__container">
  ...
  <div class="cart-menu__product__body">
    ...
    <h4 class="heading-4">${cart[i].discount
                ? ((cart[i].price - (cart[i].price * cart[i].discount) / 100) *
                    cart[i].quantity).toFixed(2)
                : (cart[i].price * cart[i].quantity).toFixed(2)}€</h4>
  </div>
  ...
  <button data-cart-index="${
            ...
            cart.indexOf(cart[i])}" class="js-delete-cart-product delete-btn">
    ...
    <h3 class="heading-3--sans">Total: <span class="total-cost">${total.toFixed(2)}</span>€</h3>
...
function deleteProductPurchased(array) {
    for (let i = 0; i < array.length; i++) {
        const buttonItem = array[i];
        buttonItem.addEventListener("click", (e) => {
            // Declaramos la variable con la que guardamos el índice del producto al que pertenece cada botón
            const element = e.target;
            let cartIndex = Number(element.parentElement.getAttribute("data-cart-index"));
